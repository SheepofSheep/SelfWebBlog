  ...
  <div class="calendar-page page-width">
    <header>
      ...
      <div class="month-control">
        <button type="button" aria-label="上个月" title="上个月" @click="moveMonth(-1)"><ChevronLeft :size="18" /></button>
        <strong>{{ title }}</strong>
        <button type="button" aria-label="下个月" title="下个月" @click="moveMonth(1)"><ChevronRight :size="18" /></button>
      </div>
    </header>
    ...
    <section v-if="selected" class="selected-day">
      <h2>{{ selected.date }} <small>{{ selected.count }} 篇</small></h2>
      <button v-for="post in selected.posts" :key="post.id" type="button" @click="router.push(`/post/${post.id}`)">

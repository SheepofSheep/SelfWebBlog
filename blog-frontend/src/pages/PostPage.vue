...
async function loadComments() {
  ...
  try {
    commentPage.value = 1
    const result = await getComments(postId.value, 1, 10)
    comments.value = result.records
    commentHasMore.value = result.current < result.pages
  } catch { comments.value = [] }
}

async function loadMoreComments() {
  if (!postId.value || commentLoadingMore.value) return
  commentLoadingMore.value = true
  try {
    const nextPage = commentPage.value + 1
    const result = await getComments(postId.value, nextPage, 10)
    comments.value.push(...result.records)
    commentPage.value = nextPage
    commentHasMore.value = result.current < result.pages
  } catch (e) {
    showToast('加载更多评论失败：' + e.message)
  } finally { commentLoadingMore.value = false }
}
  ...
  <div class="post-page">
    <div v-if="post" class="post-container glass-card">
      ...
      <div class="comments-section">
        ...
        <div v-if="commentHasMore && comments.length > 0" class="load-more-wrap">
          <button class="pill-btn pill-btn-ghost" :disabled="commentLoadingMore" @click="loadMoreComments">
...
.pill-btn-ghost {
  border: 1px solid rgba(244, 164, 184, 0.4);
  background: rgba(255, 255, 255, 0.35);
  color: var(--theme-pink-hover);
  padding: 8px 24px;
  border-radius: var(--radius-pill);
...
.pill-btn-ghost:disabled { opacity: 0.5; cursor: default; transform: none; }
